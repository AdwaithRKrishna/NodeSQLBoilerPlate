...
module.exports.createCompany = async (req, res, next) => {
  try {
    const company = await Company.create({ name: req.body.name }).then(
...
module.exports.readCompany = async (req, res, next) => {
  ...
  try {
    const company = await Company.findByPk(companyId).then(
...
module.exports.updateCompany = async (req, res, next) => {
  ...
  try {
    ...
    const updatedCompany = await company.update({ name: companyName }).then(
...
module.exports.deleteCompany = async (req, res, next) => {
  const companyId = req.params.id
  try {
    let company = await Company.findByPk(companyId).then(
      throwIf(r => !r, 400, 'not found', 'Company Not Found'),
      throwError(500, 'Sequelize error')
    )
